"""Query all bank API endpoints and print a sample response.
...
def main():
    print("Querying each bank API (GET /accounts?limit=1)...")
    ...
    for i, bank in enumerate(BANKS):
        ...
        try:
            ...
            sample = accounts[0] if accounts else None
            count = len(accounts)
        except Exception as e:
            sample = f"ERROR: {e}"
            count = 0

        print(f"{bank:22} (port {port}): {count} accounts, sample: {sample}")
